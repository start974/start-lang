use super::ast::*;
use super::error::ErrorKeyword;
use crate::utils::error::Error;
use crate::utils::location::{SourceCache, SourceId, UNKNOWN_LOCATION};
...
impl Parser {
    ...
    /// parse
    pub fn parse(&self) -> Result<Program, impl Error> {
        let node = self.tree.root_node();
        self.parse_program(&node)
    }

    pub fn parse_program(&self, _node: &Node) -> Result<Program, impl Error> {
        Err(ErrorKeyword::new("test", UNKNOWN_LOCATION))
    }
}
